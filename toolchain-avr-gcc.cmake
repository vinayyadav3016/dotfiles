...
function(add_avr_executable EXECUTABLE_NAME)
    ...
    foreach(mcu ${target_mcus})
        ...
        set(hex_file ${EXECUTABLE_OUTPUT_PATH}/${EXECUTABLE_NAME}-${mcu}.hex)
        set(map_file ${EXECUTABLE_OUTPUT_PATH}/${EXECUTABLE_NAME}-${mcu}.map)
        ...
        add_custom_command(
            OUTPUT ${hex_file}
            COMMAND
                ${AVR_OBJCOPY} -j .text -j .data -O ihex ${EXECUTABLE_OUTPUT_PATH}/${elf_file} ${hex_file}
            COMMAND
                ${AVR_SIZE_TOOL} ${EXECUTABLE_OUTPUT_PATH}/${elf_file}
            DEPENDS ${elf_file}
        )
        list(APPEND all_hex_files ${hex_file})
        list(APPEND all_map_files ${map_file})
    endforeach(mcu ${target_mcus})
    set(eeprom_image ${EXECUTABLE_OUTPUT_PATH}/${EXECUTABLE_NAME}-eeprom.hex)
    add_custom_command(
        OUTPUT ${eeprom_image}
        COMMAND
            ${AVR_OBJCOPY} -j .eeprom --change-section-lma .eeprom=0 
                -O ihex ${EXECUTABLE_OUTPUT_PATH}/${elf_file} ${eeprom_image}
        DEPENDS ${EXECUTABLE_OUTPUT_PATH}/${elf_file}
    )
